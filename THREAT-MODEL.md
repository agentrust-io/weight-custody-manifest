...
- **T1.2 Read weights out of GPU memory.** Against a *software* ADV-1/ADV-2 (privileged host or hypervisor, no physical access): mitigated by NVIDIA CC access-control firewalling (TCB item 2, design 3.6). Against a *physical* ADV-1/ADV-2 (operator willing to mount on-package attacks): **NOT mitigated cryptographically.** Weights are plaintext in HBM and NVIDIA scopes sophisticated physical attacks out. Residual is handled by the design-3.6 stack, not by silicon: raise cost (decapsulation, on-package probing), detect and attribute (attestation plus receipts), contain (wipe-on-lapse, revocation), legal recourse, and the optional `physical_hardening` tier. This is the honest downgrade from custody to cost-plus-detection against the physical operator, and it is the single most important correction the GPU assessment forced. **A parallel question is open (8.6): if TEE.fail-class attacks also defeat the CPU CVM (TCB item 1), the same downgrade applies to T3.3 and to the key, not only to weights.** **Correction (2026-09-11): this threat was priced too high.** It assumed reaching GPU-resident weights requires decapsulation or on-package probing. The cheaper path does not touch the GPU: NVIDIA's attestation does not identify the guest it serves, so an adversary who has broken the CPU TEE relays the attestation call to a genuine confidential GPU elsewhere (TEE.fail demonstrated a local RTX 3060 forwarding to an external H100, passing both TDX and CC attestation). WCM's composite nonce binding (SPEC 3.2) defends this shape specifically, because a report echoing another session's nonce fails the gate; it does not survive T1.7, where the adversary holds the CPU attestation key and therefore controls both halves.
...
- **T1.9 Alias DRAM to defeat the launch measurement (BadRAM-class), now partially gated.** A ~$10 SPD spoof aliases DRAM so a measurement reads clean while the guest runs from aliased addresses. Two independent controls now apply. WCM's own `memory_fingerprint_challenge` (T1.7 discussion, SPEC 3.1) detects the resulting address collisions at release time. Separately, AMD ships a boot-time alias scan whose result is reported in the attestation report itself (`PLATFORM_INFO` bit 5, `ALIAS_CHECK_COMPLETE`, per AMD-SB-3015), and `release_policy.platform_integrity.alias_check_complete: required` makes it a gate condition. This is the only *hardware-reported* statement about physical platform state that any production attestation report carries. Residual: both are time-of-check/time-of-use against a hardware adversary. Battering RAM (IEEE S&P 2026, ~$50) passes the command and address lines through untouched during POST so the boot scan reports success, then enables aliasing at runtime. Requiring the bit raises the floor from an SPD spoof to an interposer; it does not close the class, and it must not be described as doing so.
...
- **T4.4 Relay a live, valid quote onto an attacker-terminated channel to divert key release (A2).** Distinct from T4.1: the quote is genuine and fresh, so nonce binding does not catch it; the adversary terminates the enclave's channel and forwards the quote to obtain the key on its own side (the intra-handshake binding gap, CVE-2026-33697; attested-TLS binds evidence to the ephemeral key, not the whole channel). Mitigation: channel binding (design 3.2). The enclave folds its transport public key into REPORT_DATA under the nonce, so a relay cannot substitute its own key without failing verification, and the KBS seals the released key to that transport key rather than returning it in the clear, so a faithfully-relayed quote yields only ciphertext the relay cannot open. Residual: on platforms where REPORT_DATA is not guest-controlled (Azure vTPM paravisor), the transport key's hardware binding rides the enclosing vTPM quote; the object-seal still holds since it rests on the transport private key staying in the enclave. Does not address a hardware owner who forges the quote outright (T1.7 / open question 8.8).
