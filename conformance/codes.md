...
> **Declared, not yet vectored.** These codes are specified so an implementation and its reports can use them, but this level has no vector corpus yet, so no implementation can currently be *scored* on it. See `conformance/README.md`.
