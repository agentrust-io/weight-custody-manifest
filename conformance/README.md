...
The offset is required and must be a non-negative integer; the runner does not
supply a default or coerce strings or fractional values. With the currently
supported report formats, NVIDIA requires `nonce-echo`, while SEV-SNP and TDX
reject it. The schema and runner both enforce this compatibility. An echo
binding cannot also claim a transport-key binding.
