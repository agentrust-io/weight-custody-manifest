...
**L2 and L3 have no vectors.** Their requirements are written below and their
error codes are allocated in [`codes.md`](codes.md), but no implementation can be
scored on them today. L1 and L4 are checks over documents, which reduce cleanly
to static files. L2 and L3 are protocol behaviour over live state, single-use
nonces, released key material, clocks, and operation counters, which does not:
expressing them needs a scripted-scenario format with deterministic seeds, and
that format is not designed yet.

So a green run means *the levels it names*, and the runner prints which levels it
did not cover. **Passing L1 and L4 is not conformance to WCM.** It is conformance
to the manifest and lineage layers, which is worth stating precisely and worth
nothing if overstated.
...
*No vectors yet.* Codes `WCM-L2-*` are allocated.
...
*No vectors yet.* Codes `WCM-L3-*` are allocated.
...
The signature vectors use fixed Ed25519 keys derived from constant seeds, so they
are reproducible and contain no secret material worth protecting. They are test
