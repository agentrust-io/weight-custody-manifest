...
Verbs:
  wcm keygen                       Generate an Ed25519 key pair.
  wcm sign    MANIFEST --role R --signer S --key PRIV   Append a signature.
  wcm verify  MANIFEST --key PUB [--key PUB ...]        Verify joint signatures.
...
def cmd_keygen(args: argparse.Namespace) -> int:
    ...
    print(
        "\nKeep private_key_b64url secret. Share public_key_b64url and key_id.",
        file=sys.stderr,
    )
    return 0
...
def cmd_sign(args: argparse.Namespace) -> int:
    ...
    kp = ed25519_from_private_b64url(args.key)
    signer = Ed25519Signer(kp)
    block = signer.sign(manifest.unsigned_dict(), role=args.role, signer=args.signer)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_sign.add_argument("--key", required=True, help="Signer private key (base64url)")
    ...
    p_verify.add_argument(
        ...
        action="append",
        required=True,
        help="A trusted public key (base64url); repeat for multiple",
    )
