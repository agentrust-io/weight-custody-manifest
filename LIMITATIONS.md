...
- **Not a hardware root of trust.** The SDK's authority-layer checks (signatures, lineage) and the KBS gate are only *cryptographic* about the runtime when a real quote verifier is wired. AMD SEV-SNP quote verification is implemented and hardware-validated. GPU-side (NVIDIA) quote verification is also implemented (`NvidiaGpuVerifier`: device chain to NVIDIA's device-identity CA, ECDSA P-384 report signature, raw challenge nonce at offset 4) and validated against a live H100 capture, independently re-run on an H200. The gate checks evidence cryptographically only when a verifier is configured. A manifest that does not set `release_policy.require_evidence_verification: false` requires it, so a KBS without a verifier refuses; with the explicit waiver, `cpu_quote_verified` and `gpu_report_verified` report structural trust only and say so in their reason. GPU confidential-compute mode is established only by a verified GPU report, on an assumption SPEC 3.2 states and bounds.
...
Azure SEV-SNP CVMs run behind a Hyper-V paravisor: there is no `/dev/sev-guest`, and the SNP report is read from the vTPM NV index `0x01400001` (HCL-wrapped). The guest does **not** control `REPORT_DATA` - the paravisor binds it to the vTPM runtime-data hash - so the SNP report alone carries no caller nonce. `AzureSnpVtpmProvider` adds the freshness: a TPM quote under the HCL-authenticated attestation key over PCR 23, with `sha256(nonce || transport key)` as qualifying data, which `AzureSnpVtpmVerifier` checks. Use `AzureSnpVtpmProvider`, not the bare-metal `SevSnpProvider`. `wcm verify-quote --kind snp` without a nonce checks the report chain and signature only, not that TPM quote. This path was validated against a live Azure SEV-SNP VM. The bare-metal `/dev/sev-guest` path is validated on a live GCP N2D guest and the bare-metal TDX report path on a live GCP C3 guest, where converting that TDREPORT into a remotely verifiable quote stays provisional. The NVIDIA GPU path is validated against a live H100 capture and independently re-run on an H200.

**Azure TDX has no freshness binding yet.** `AzureTdxVtpmProvider` exchanges the TD report for a DCAP quote and stops there: it takes no nonce-bound vTPM quote, so `verify_tdx_quote` can only pass it with `expected_nonce=None`, and a captured Azure TDX quote replays. Treat that evidence as proof of a genuine TDX platform and measurement, not of a live session.
