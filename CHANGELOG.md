...
(`conformance/codes.md`), 83 language-neutral JSON vectors, and a runner exposed
...
**All four levels are vectored, 83 vectors.** L1 (manifest and joint signature,
32) and L4 (derivative lineage, 10) ask questions about documents. L2
(attestation-gated release, 29) and L3 (runtime custody, 12) ask what a system does
...
**One requirement is still uncovered and the runner says so on every run:**
cryptographic quote verification, the signature and certificate chain against a
vendor root (`WCM-L2-0011`, `WCM-L2-0012`). It needs raw hardware evidence plus
trust anchors rather than the declarative evidence the gate vectors carry, and an
honest way to express nonce binding given that the Azure SEV-SNP capture binds the
vTPM attestation key in `REPORT_DATA` rather than a caller nonce. Both codes are
listed in `NOT_YET_VECTORED_CODES`, marked in `codes.md`, and printed by
`wcm conformance`. A passing L2 means the gate enforces the manifest's policy; it
does not mean the gate can tell a genuine quote from a fabricated one. A test
asserts that exemption list does not grow, so "add it to the exempt set" cannot
become the way a failing code disappears.
