...
- **Now** - pre-1.0 developer preview, with the full attestation matrix validated
  on real silicon: AMD SEV-SNP (Azure), Intel TDX (GCP) and NVIDIA H100 CC, each
  with a committed fixture that verifies offline.
- **Next** - bare-metal `/dev/*-guest` provider validation, and vendor-format
  conformance vectors that carry real captured evidence.
